...
pyinstaller --onedir --windowed --name "BulkFileOrganizer" `
...
```
dist/
├── BulkFileOrganizer/          # Portable folder (run BulkFileOrganizer.exe)
├── BulkFileOrganizer.zip       # Compressed portable archive
└── BulkFileOrganizer-1.0.0.msi # Windows Installer (if MSI build enabled)
```
...
├── BulkFileOrganizer.spec    # PyInstaller spec file (for `pyinstaller BulkFileOrganizer.spec`)
